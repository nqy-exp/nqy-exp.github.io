...
**Subject**: Micrometer; Glycerin and nail base coat blank groups
**Sample ID**: T000 
**Environmental Conditions**: 28.6°C, 65% RH
...
T000-G (Glycerin group): Bubbles are present, but at a low density. 
...
**Subject**: Longitudinal and cross-sectional morphology of cotton batt
**Sample ID**: T001 
**Environmental Conditions**: 27.4°C, 63% RH
...
T001-LG (Glycerin group): The image is clear, and the natural twisted characteristics of the cotton fibers are clearly visible. Although vignetting exists, it does not affect observation. 
T001-LZ (Nail base coat group): The image is slightly blurrier than the glycerin group, but the longitudinal structure remains consistent with the glycerin group. According to FIJI measurements, the cotton fiber diameters are both 0.01–0.02 mm, indicating that the nail base coat caused no morphological changes or damage to the fibers.

Cross-section: 
This was the **most significant challenge** in this experiment. 
...
**Subject**: Longitudinal and cross-sectional morphology of cotton plain weave greige fabric
**Sample ID**: T002
...
<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/T002.jpg" alt="Figure 4: T002 Pure cotton plain weave greige fabric" style="max-width: 400px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
...
During longitudinal preparation, only part of the fibers were pulled from the cotton thread before being placed on the slide to observe the difference between the thread and individual fibers. 
...
T002-LG (Glycerin group): The cotton fiber structure is clear. In Figure 02, the edge structure of the cotton thread can be observed after increasing the brightness.
...
在显微镜图像中可以看到，两组图像都非常干净，环境粉尘对实验的影响较小。视野范围内无明显粉尘纤维。
T000-G (甘油组)：有气泡，但密度较低。
...
在研究计划中，台灯为光源，最大亮度的照度为1250lux。
但是实际操作中，这个亮度太高，导致即使显微镜中放置滤光片，调节光圈至最小，肉眼也无法仔细观察，同时摄像会出现频繁的过曝。
...
由于手机照相功能（IOS系统自带--相机APP）过于智能，会自己调整微距、打开夜间模式以及曝光，所以在观察摄像时出现跳焦黑屏现象。
应对方案：在设置-相机-保留设置中打开微距控制、夜间模式以及曝光调整。

拍摄前，关闭夜间模式，点击0.5x进入广角镜头模式自动开启微距后，长按屏幕中的目镜取景框，打开自动曝光/自动对焦锁定功能，稳定成像环境后，在逐步从1.0x到2.0x的放大。如果随后还是跳焦，则重新回到0.5x进行重复操作。
调整显微镜粗焦阶段很容易出现跳焦情况，可以现在0.5x的广角镜头中进行粗准焦的调整，出现模糊轮廓后再逐步换到2.0x。
...
纵向结构：
T001-LG（甘油组）：成相清晰，棉纤维的天然转曲特征清晰可见。光晕问题存在，但不影响观察。
...
在纵向结构制样时，棉线只拉扯出部分纤维后即放在载玻片上，观察棉线及棉纤维的区别。
...
T002-LG（甘油组）：棉纤维结构清晰。图02在增加亮度后，可以看到棉线边缘结构。
...
由于物镜40x的观察与其他图片一致，而物镜10x的时候更能观察到问题，所以只拍摄了物镜10x的图片。
...
可以看到图像略模糊（可能是由于拍摄时的抖动），但是棉纤维与T002-LG-01相比，并没有显著改变，可见指甲底油未对棉纤维产生形态上的白化影响或者物理破坏。
