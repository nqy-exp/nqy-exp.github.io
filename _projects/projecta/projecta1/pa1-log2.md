...
type: log
date: 2026-07-30
description: "Exploring cross-section preparation methods for cotton fibers. This log planned to investigate the use of various embedding media and parchment paper skeletons to optimize microscopic imaging clarity, but through experimentation, a persistent issue of cross-sectional whitening was discovered."
---

## Overview:
This log records the experimental plan and numbering system for Special Experiment SE01 (Cross-sectional Sample Preparation Exploration).

Experiments found that the various embedding media selected so far have no effect on the longitudinal structure of the T002 cotton fibers. Parchment paper (*Baking paper*) performs excellently as a cutting skeleton; however, because it obstructs the microscopic field of view, it is not suitable to be used alone as an embedding skeleton without prior treatment. Based on this, an optimized scheme for cross-sectional slicing is proposed, and the subsequent experimental directions are planned.

Additionally, regarding the observed cross-sectional whitening, a hypothesis is proposed: the hydrophobic wax layer of the cotton fiber's primary wall attracts micro-bubbles, which then cause light scattering after solidification, resulting in blurred imaging.

*Note: After uploading Log01, I discovered an Abbe condenser while cleaning the microscope. It allows for adjusting the height of light convergence. Its subsequent use has slightly improved image quality, though the effect remains relatively limited.*
...
Consequently, I have decided to conduct a dedicated special experiment (Code: SE01) specifically for cross-sectional sample preparation, with T002 cotton fibers as the subject. If existing schemes prove unsuitable for other fiber types in the future, additional special experiments will be conducted.

## Special Experiment (SE01) — Exploration of Cross-sectional Sample Preparation Methods

**Objectives:**
To explore and determine the most suitable embedding media and slicing schemes for preparing cross-sectional images of cotton fibers under existing experimental conditions.

Previous schemes were limited in the types of selectable embedding agents due to high cleanliness requirements for the slicer. This experiment plans to expand the range of media through the "embedding and solidification outside the slicer" method, and attempts to use Parchment paper(*Baking paper*) as a skeleton for fiber cutting.

**Subject**: T002 Cotton Fiber

**Rationale for selection:**
Cotton is the most commonly used natural fiber material in daily life. Furthermore, among all current materials, only cotton grey fabric is available; since it is untreated, it is suitable as a baseline material for the initial exploration of sample preparation methods.
...
- 2.Effect of different embedding media on parchment paper (the carrier material).
  Thinly coating the parchment paper with embedding media, rolling it up, slicing, and observing under a microscope. The embedding media used in this special experiment include:
    ...
    - 2-in-1 nail polish (lellmoll)
    - Fabric hardener - White (Kaying)
    - Fabric hardener - Clear (Kaying)
    - 502 Glue (deli)
    - Edge sealing glue (KEARING)
    - Edge sealing liquid (KEARING)
    - UV Glue (Anergoo) 


<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/emb-sum.jpg" alt="Figure 1: Summary of used embedding media" style="max-width: 400px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 1: Summary of used embedding media
  </figcaption>
</figure>
...
***This log is for a special experiment and uses a specialized numbering system, which is separate from the common project numbering system.***

**Numbering Structure:** 
- For parchment paper-only experiments (without fibers): 
  BPC/L-XX-00 
  (Parchment paper image structure - embedding medium - number) 
  
  *BB was introduced after the experiment on 2026.07.28: BB - baked parchment paper.*

- For cross-sectional cutting-effect experiments: 
  T02C/L-XX-YY-00 
  (Cotton fiber image structure - embedding medium - treatment method - number)

**Numbering Abbreviations:** 
- C/L: 
	L: Longitudinal structure 
	C: Cross-section

- XX (Embedding media): 
	- ZB: Nail base coat 
	- ZT: Nail top coat 
	- ZZ: 2-in-1 nail polish 
	- HW: Fabric hardener - White 
	- HT: Fabric hardener - Clear 
	- J5: 502 Glue 
	- FW: Edge sealing liquid 
	- FA: Edge sealing glue 
	- UV: UV Glue 
...
Cut the parchment paper into strips of the required width. Since I am using double-sided circular parchment paper for meat patties (Brand: Meichu), one edge must first be trimmed to make it flat. Subsequently, with the assistance of tweezers, roll it up at 5mm intervals, place it into the slicer and press it tightly, then cut off the excess part (_**Caution: Operate safely!**_), place it on the sample holder, and perform slicing to complete the sample preparation. 

<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/y172-bp.jpg" alt="Figure 2: Cross-sectional sample preparation process of parchment paper " style="max-width: 400px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 2: Cross-sectional sample preparation process of parchment paper 
  </figcaption>
</figure>
...
In conclusion, all tested embedding media had **no effect** on the longitudinal structure of T002 cotton fibers.

<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/T02L-Psum.jpg" alt="Figure 3: Longitudinal structure of cotton fibers under different embedding media" style="max-width: 800px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 3: Longitudinal structure of cotton fibers under different embedding media
  </figcaption>
</figure>


Unexpectedly, it was observed that T02L-UV-02 (a shot with relatively good quality) showed poor imaging performance of UV glue in terms of longitudinal structure; the overall image was slightly blurry, which is suspected to be due to the thickness of the colloid during solidification. However, the solidification effect met expectations, being relatively clear and rigid, with fewer bubbles compared to other embedding media. The performance of this medium in cross-sectional experiments is expected.

Based on known components, the current embedding media are mainly classified into the following categories:

- Nitrocellulose-based media: Nail polish types
- Acrylate copolymers: Nail base coat (KH Xiaodengpao), fabric hardener types
- Cyanoacrylate media: 502 Glue
- Acrylate monomers or oligomers: UV glue

In other words, the primary components of current embedding media are mainly acrylates, which do not virtually cause damage to the cotton fiber structure.
...
<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/T02L-ZT-Bubble.jpg" alt="Figure 4: Large amount of microbubbles attached around cotton fibers in nail top coat T02L-ZT-01/02/03; compared with T02L-ZT-04 which has no bubbles after natural solidification" style="max-width: 800px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 4: Large amount of microbubbles attached around cotton fibers in nail top coat T02L-ZT-01/02/03; compared with T02L-ZT-04 which has no bubbles after natural solidification
  </figcaption>
</figure>


As seen in the image, fine microbubbles are clearly attached to the cotton fibers. In contrast, 04 used natural drying and showed no bubbles. This indicates that using a dust blower to accelerate solidification has a negative impact on embedding quality; therefore, blowing air should not be used to accelerate solidification.

This reminds me of the whitening of cross-sections in previous T001 and T002 experiments. It can be inferred that using a dust blower to accelerate solidification caused **fine micro-bubbles** to form around the fibers, creating a layer of tiny structures that acted as a scattering layer, making the cross-section unclear.

Considering this issue, no blowing devices should be used to accelerate solidification in future experiments; instead, natural solidification should be allowed to reduce the influence of micro-bubbles on observation.
...
<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/T02L-HW.jpg" alt="Figure 5: Different images of the same area for T02L-HW and T02L-NG under different light intensities" style="max-width: 800px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 5: Different images of the same area for T02L-HW and T02L-NG under different light intensities
  </figcaption>
</figure>


Observation results showed that in the glycerin group, white areas appeared under both bright and dim light; thus, **these white areas are not an indicator of fiber damage**. Combined with the fact that the fibers in T02L-HW appear intact with natural twists and longitudinal structure characteristics, it is concluded that the embedding medium—fabric hardener (white)—does not cause damage to the cotton fibers.

Regarding this phenomenon, I hypothesize two possible causes:
- The inherent structure of the cotton fiber (e.g., the wax layer or cellulose) caused scattering of the light from above.
...
This experiment mainly tested the use of parchment paper as a cutting skeleton for cotton fiber cross-sections, as well as the effect of embedding media on the parchment paper.

First, in the **absence of embedding media**, experiments were conducted on slicing parchment paper alone, and on slicing parchment paper wrapped around cotton fibers.

Subsequently, using nail base coat and nail top coat as embedding media, experiments were performed on slicing parchment paper coated with the medium alone, as well as slicing fibers that were embedded and solidified on the parchment paper after being rolled up (PP). The experiment primarily focused on using nail top coat as the embedding medium.

**Experimental Analysis:**

This experiment encountered <strong class="wow">significant difficulties!</strong>
...
<figure style="text-align: center; margin: 2em 0;">
  <img src="/images/BPC-T02C-NG.jpg" alt="Figure 6: Microscopic images of parchment paper-only experiments" style="max-width: 800px; height: auto; border-radius: 4px;">
  <figcaption style="font-size: 0.85em; color: #555; font-style: italic; margin-top: 10px;">
    Figure 6: Microscopic images of parchment paper-only experiments
  </figcaption>
</figure>
...
Next, experiments were conducted by applying a thin layer of either nail base coat or nail top coat onto the parchment paper, followed by rolling it up for slicing and observation after solidification. 
...
It is hypothesized that since the nail polish series serves as an embedding medium, while it provides strong support and a chemical composition closest to nitrocellulose solution, its components are overly complex due to the addition of various chemicals (such as those added for color or fragrance). This complexity leads to **unstable optical properties** after solidification and slicing, such as stress whitening or scattering caused by porous structures (<strong class="wow">e.g., introduced bubbles</strong>), ultimately resulting in a white, blurry field of view.

### Summary of this experiment

For the SE01-01 experiment so far, the following problems have been resolved:

- The currently selected embedding media all have no effect on the cotton fiber structure.
- The feasibility of using parchment paper as a cutting skeleton for cross-sections has been confirmed.
...
There are still certain expectations for UV glue as an embedding medium. However, due to its characteristics, it must be tested whether it will experience micro-structure scattering and stress whitening similar to nail polish during solidification. Furthermore, if UV glue is applied thickly, there may be **difficulties in slicing.**
...
- Difficult to use for identifying unknown fibers
- Increases the difficulty of marking and identification in Phase 2
- In Phase 3, wool fibers may cause interference.
Consequently, Option 3 can only be used for cross-sectional slicing of known samples to observe and understand their structures.
...
As a purely curiosity-driven exploration project, I have no time constraints, my goal is to practice and verify as many ideas as possible. ***If all methods are attempted without finding an effective cross-sectional sample preparation method, I will have to shelve the project for now.***

*Note: During this stage, the experimental numbering system was perfected.*

<em style="color: #e67e22;">Additionally, due to currently high temperatures and the fact that the experiment involves items with strong-smelling materials like nail polish, window ventilation is required. Under these difficult conditions, progress has been somewhat slow.</em>
...
## 概述：
本日志记录了特别实验 SE01（横截面制样探索）的实验计划与编号体系。

实验发现，目前选择的各类包埋剂对 T002 棉纤维的纵向结构均无影响。硅油纸(*烘培纸*)作为切割骨架表现很出色，但因其会遮挡显微镜视野，不适合在未处理的情况下单独作为包埋骨架。基于此，提出优化横截面切片的方案，并规划了接下来的实验方向。

此外，针对观察到的横截面发白问题，本文提出了一个假设：即棉纤维初生层蜡质形成的疏水层吸引了微小气泡，固化后导致光线散射，使成像模糊。

*注：在Log01上传之后，我对显微镜进行清洁时才发现显微镜具有阿贝聚光镜，可调整光源聚集的高度，后续加入使用，图像质量略有改善，但效果相对有限。*
...
- 3.不同包埋方式对于横截面切割的影响。
	...
	- 不使用包埋剂，直接将纤维与硅油纸卷起——切片后观察
	- ~~不使用包埋剂，直接将纤维与硅油纸卷起——切片后移走硅油纸后观察~~（*已放弃。经尝试，切片后硅油纸与纤维紧密结合，剥离难度大且极易造成纤维损耗。*）

### **编号缩写**：

***本Log为特别实验，使用特别版编号，不与项目使用共同编号体系。***
...
- 对于硅油纸单独实验（无纤维）的编号结构：
  ...
  *在2026.07.28的实验后引入BB：烘烤过的硅油纸*

- 对于横截面切割影响实验：
  T02C/L-XX-YY-00 
  （棉纤维图像结构-包埋介质-处理方式-编号）
...
- YY （处理方式）
	- PA: 在硅油纸上涂包埋剂——固化后取出纤维——放入切片器切片后观察
	- PN: 在硅油纸上涂包埋剂——固化后取出纤维卷入**新的硅油纸**——放入切片器切片后观察
	- PP: 在硅油纸上涂包埋剂——固化后连同硅油纸一同卷起——放入切片器切片后观察
	- PS: 不使用包埋剂，直接将纤维与硅油纸卷起——切片后观察 （空白对照组）
...
根据已知的成分进行分类，目前包埋剂主要分为以下几类：

- 硝化纤维类介质：指甲油类
- 丙烯酸（酯）共聚物：指甲底油（KH小灯泡）、织物硬化剂类
- 氰基丙烯酸酯类介质：502胶水
- 丙烯酸酯类单体或低聚物：UV无影胶

也就是说，目前的包埋剂基本成分均是以丙烯酸酯为主，对棉纤维结构基本不产生破坏。
...
考虑到该问题，以后在实验中，不应使用任何吹风设备来加速固化，而是等其自然固化，来减少微小气泡对观测形成的影响。
...
关于该现象，我推测其原因可能有二：
- 棉纤维的固有结构（例如蜡制层或纤维素）对上方光线产生了散射
- 棉纤维在该位置的纵向厚度较大（例如由于天然转曲导致纤维沿 Z 轴折叠），导致光线无法穿透而形成的白色视觉区域。
...
本次实验遇到了<strong class="wow">很大的困难</strong>！
...
猜测是因为指甲油系列作为包埋介质，虽然能够提供强有力的支撑以及最接近火棉胶的化学介质，但是由于增加指甲油各类效果的化学物质过多（例如为了增加色彩、香味等效果添加的化学物质），成分过于复杂，导致其固化切割后出现了**光学特性**不稳定的情况，比如出现应力白化，或者多孔隙（<strong class="wow">比如引入气泡</strong>）结构导致出现散射，最终导致视野中呈现白茫茫一片的情况。

### 本次实验小结

SE01-01实验到目前，已经解决的问题有：
- 目前选择的包埋剂均对棉纤维结构无影响
- 证实了硅油纸作为横截面切割骨架的可行性
...
作为一个纯粹出于好奇心的探索项目，我没有时间限制，目标是尽可能地实践并验证所有的想法。***如果所有方式都尝试过，也找不到可以得到观测良好的横截面制样方式，那只能先对其进行搁置。***

*备注：在这个阶段，完善了实验编号命名体系。*

<em style="color: #e67e22;">另，目前气温过高，实验本身涉及指甲油等气味浓重物品，需开窗通风，条件艰难，故进展略微缓慢。</em>
